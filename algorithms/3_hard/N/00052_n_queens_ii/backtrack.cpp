class Solution {
    static bool __attribute__((always_inline)) is_safe(const vector<vector<char>> &board, const size_t len, const size_t i, const size_t j) {
    ...
    static void backtrack(int &count, vector<vector<char>> &board, const size_t len, const size_t curr_row) {
        if (curr_row == len) {
            count += 1;
            return;
        }
